...
public class StuDeck {
	...
	void update() {
		...
		for(int i=0;i<list.size();i++) {
			...
			if(temp == 0) {
				System.out.printf("%s 학생의 정보가 없습니다. 다시입력해주세요.\n",uName);
				System.out.println();
				break;
			}
		}
	...
	void remove() {
		...
		for(int i=0;i<list.size();i++) {
			...
			if(s.getName().equals(rName)) {
				...
				case 1:
					list.remove(i);
					System.out.printf("%s 학생의 정보가 삭제되었습니다.",rName);
					break;
	...
	void rSort() {
		...
		System.out.println("등수정렬이 완료되었습니다.");
		System.out.println();
		System.out.println("--------------------------------------------------------------");
		System.out.printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
				title[0],title[1],title[2],	title[3],title[4],title[5],title[6],title[7]);
	...
	void save() throws Exception{
		...
		// 파일 확인 및 생성
		File file = new File(folder+"StuScore11.txt");
		...
		// 파일 내용 넣기
		FileWriter fw = new FileWriter("c:/save/StuScore11.txt");
